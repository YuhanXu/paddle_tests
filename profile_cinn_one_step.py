"""
单步 CINN 训练 + nsys profile
使用小型 Llama (168M) 模型，执行 warmup + 1 step 训练
"""
import sys
...
if __name__ == "__main__":
    ...
    # Profiled step: 用 CUDA profiler 标记
    print("--[Profile] Starting nsys-profiled step...")
    paddle.device.synchronize()
    ...
    paddle.device.cuda.synchronize(0)
    profiler_end = time.time()

    print(f"--[Profile] Profiled step done. loss={loss.item():.6f}, "
          f"time={((profiler_end - profiler_start) * 1000):.2f} ms")
    print("--[Done]")
